...
object LicenseManager {
    ...
    /** 开发模式：设为 true 跳过授权验证，直接进入主界面 */
    const val DEBUG_BYPASS = true
    ...
    init {
        try {
            ...
            android.util.Log.w("LicenseManager", "libaimbot_license.so not loaded: ${t.message}")
            nativeLoaded = false
        }
    }
    ...
    /** 开发模式跳过验证，返回一个长期有效的假授权 */
    private fun debugBypassResult(): VerifyResult {
        ...
        return VerifyResult.Success(
            LicenseInfo(
                ...
                hours = 99999,
                version = VERSION,
                expiryTs = now + 99999 * 3600L,
                code = "DEBUG-BYPASS-MODE"
            )
        )
    }
    ...
    fun verifyAndPersist(context: Context, inputCode: String): VerifyResult {
        if (DEBUG_BYPASS) return debugBypassResult()
        if (!nativeLoaded) return VerifyResult.Failure("授权模块未加载")
    ...
    fun clearSaved(context: Context) {
        ...
        try { nativeClearStored(context) } catch (_: Throwable) {}
    }
     ...
     */
    fun getServerTimeSec(): Long {
        if (!nativeLoaded) return System.currentTimeMillis() / 1000L
        return try { nativeGetServerTime() } catch (_: Throwable) { -1L }
    }
}
